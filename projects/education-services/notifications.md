...
이는 당시 선택 기준입니다. 실제 처리량, 비용 절감 수치, TTL 적용 기간은 확인되지 않아 성과로 적지 않았습니다.
...
중복 검사와 발송의 논리적 순서입니다. 실제 Valkey 명령과 동시 요청 처리 방식, TTL 길이, 발송 실패 시 해시 처리 방식은 확인되지 않아 구체적으로 적지 않았습니다.
...
- 비용과 운영 관리를 고려해 MongoDB의 시간 순서 데이터 저장 방식 대신 DynamoDB를 선택했다.
...
발송 전에는 위의 Valkey 해시 검사로 반복 요청을 걸러냈습니다. 전체 구성도와 중복 검사 흐름은 나눠 표시했습니다. 요청 보관과 큐 적재의 정확한 선후 관계, 재시도 횟수는 확인 필요입니다.
...
- 개발 서버와 운영 서버에 배포했다. 발송량이나 성능 개선 수치는 확인되지 않았다.
