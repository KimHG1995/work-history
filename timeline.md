# 기간별 기록

[전체 안내](README.md)로 돌아갑니다.

## 근무 기간

| 기간 | 역할 | 주요 작업 |
| --- | --- | --- |
| 2020-12 ~ 2021-06 (6개월) | 인턴 | [TTS 발음 교정과 웹 서비스 유지보수](experience.md#tts-서비스-기업) |
| 2021-07 ~ 2024-01 | 백엔드 개발, 인터널팀 리드 | [단기 구인구직 플랫폼 운영 개선](projects/platform-operations/README.md) |
| 2024-01 ~ 2025-10 | 백엔드 개발, 대리 | [교육 서비스 개발과 운영 개선](projects/education-services/README.md) |

## 작업별 기간

### 단기 구인구직 플랫폼

아래 작업은 2021-07 ~ 2024-01 근무 중 진행했습니다.
목록 순서는 작업의 시작 순서를 뜻하지 않습니다.

| 작업 | 확인한 작업 기간 |
| --- | --- |
| [초기 관리자 페이지](projects/platform-operations/initial-admin.md) | 정확한 기간 미확인 |
| [급여 정산과 환불](projects/platform-operations/settlement.md) | 약 1개월. 시작일과 종료일 미확인 |
| [관리자 화면과 API 개선](projects/platform-operations/admin-improvement.md) | 정확한 기간 미확인 |
| [데이터 분석 환경과 대시보드](projects/platform-operations/data-analytics.md) | 정확한 기간은 기억나지 않음 |
| [자동 배포](projects/platform-operations/deployment.md) | 정확한 기간 미확인 |
| [이벤트 공통 코드 정리](projects/platform-operations/event-system.md) | 정확한 기간 미확인 |

### 교육 서비스

아래 작업은 2024-01 ~ 2025-10 근무 중 진행했습니다. 목록 순서는 작업의 시작 순서를 뜻하지 않습니다.

| 작업 | 확인한 작업 기간 |
| --- | --- |
| [쿠폰 이벤트 시스템](projects/education-services/promotion.md) | 제휴 연계 시작 2025-06-02, 개발 배포 2025-06-13, 운영 배포 2025-07-28 |
| [AI 심리검사 서비스 개편](projects/education-services/assessment-renewal.md) | 합류 2025-02-25, 개발 배포 2025-03-04, 운영 배포 2025-03-06. 전체 종료일 미확인 |
| [통합 알림 시스템](projects/education-services/notifications.md) | 작업 기간 미확인. 개발 배포 2025-02-19, 운영 배포 2025-03-01 |
| [미술 교육 서비스 개편](projects/education-services/art-service-renewal.md) | 정확한 작업 기간 확인 필요 |
| [개인 고객 서비스 관리자 페이지](projects/education-services/b2c-admin.md) | 정확한 작업 기간 확인 필요 |
| [개인 고객 수강 앱 서버](projects/education-services/class-app-server.md) | 정확한 작업 기간 확인 필요 |

## 작성 방법

- 근무 기간과 작업 기간을 나눠 적습니다.
- 시작일과 종료일을 알면 `YYYY-MM-DD ~ YYYY-MM-DD`로 적습니다.
- 월까지만 알면 `YYYY-MM ~ YYYY-MM`으로 적습니다.
- 진행 중이면 종료일 대신 `진행 중`을 적습니다.
- 기간이 기억나지 않으면 추측하지 않고 그대로 남깁니다.
- 상세 내용은 작업 파일에 적고 여기에는 기간과 링크만 둡니다.
